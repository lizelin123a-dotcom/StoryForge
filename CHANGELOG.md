...
- 重建创作台 UI，不再沿用旧三栏拼接布局，改为面向作家的 `Writer Studio` 工作台：顶部显示作品、状态、字数与当前章节，主体按“AI 编辑 / 正文稿纸 / 节点审阅 / 检测监控”组织。
- 新增 [`WriterStudio.vue`](storyforge/frontend/src/components/WriterStudio.vue) 作为创作台外壳，集中管理作家写作时的主要区域，减少 App 模板中直接堆叠三块面板导致的结构混乱。
- 全局视觉系统从冷色后台风格改为更适合长时间写作的暖暗稿纸风格，建立 StoryForge 专用颜色、边框、阴影、按钮和面板样式变量。
- 重建左侧 AI 编辑面板：以“创作对话”为主，Skill、聊天流、输入框、作品资产、危险全自动开关和启动/暂停/导出操作分区呈现，避免工具按钮和资产内容混成一堆。
- 重建中间正文区：章节正文以大面积稿纸卡片呈现，采用更适合阅读和写作的 serif 字体、较大行距和更安静的背景，突出作家的主要任务是写正文。
- 重建节点草稿/审阅区：节点列表、当前节点编辑器、待审提示和审阅按钮形成固定工作流；“通过并写入正文”为主按钮，“修改后通过/回滚节点”为次级操作。
- 重建右侧检测与监控面板：统一为 Inspector 风格，检测、监控、审阅、生成逻辑、事件用横向标签切换，卡片层级和指标样式更清晰。
- 全局最小宽度提升到 1280px，匹配创作工作台的实际信息密度，避免左右工具栏压缩正文和节点审阅区。
- 这是对创作台整体使用方式和主界面的重建，按版本号规则递增第一位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.0.0`。
