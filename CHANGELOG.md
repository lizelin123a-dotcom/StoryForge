...
- 补充中英双语 README，整理项目定位、启动方式和功能说明，方便后续上传、迁移和给他人查看。
- 重构右侧“页边批注”显示逻辑：把写作时更有用的“本章要写什么、全书大纲、当前章纲、当前伏笔、角色此刻状态、读者正在等什么、章尾钩子、近章记忆、本轮改写要求”放到前台，将原来的工业化诊断收进“红笔审稿”。
- 增强章节章纲结构：章节规划新增章节功能、叙事视角、备忘、情绪设计、质量门槛；节点新增小节功能、具体写作目标、结尾落点、出场角色、微兑现等字段，让章纲更像作者可读的写作任务，而不是模型内部数据。
- 强化章纲检查和正文生成：章纲检查会识别空泛目标、AI 味、缺少落点、缺少具体写作内容等问题；节点生成会使用小节功能、写作目标、角色、微兑现和结尾落点来写正文。
- 新增章节归档、当前焦点和文本 span 映射能力：支持章节归档、近章记忆、下一章焦点、节点到正文范围映射和局部片段替换，为后续长篇连续性和局部改稿打基础。
- 新增 workflow API：提供章节归档、当前焦点、章节 span 构建、span 查询和片段 patch 接口。
- 增加多模型角色配置：规划、正文、审阅、快速任务可以分别指定模型；正文通道支持独立 DeepSeek/OpenAI-compatible Key 和 Base URL，方便用 GPT 做规划/审稿、用 DeepSeek 写正文。
- 左侧 AI 工具面板补充当前可见的模型配置入口，包括默认 Key/Base/模型、正文模型、正文 DeepSeek Key/Base 和全自动写入；AI 操作总线明确禁止修改 API Key、Base URL 和模型名。
- 案头设定改为可直接编辑和删除：用户可在案头面板手动修改已有设定，保存空值时删除对应设定，避免反复靠对话纠正 AI 理解。
- 共创 AI 新增 UI Action Bus：AI 可以提出改写节点、应用补丁、重写章节、更新当前章纲、更新全书大纲、更新/删除案头设定等可执行操作；高风险操作走前端确认，并过滤模型配置相关操作。
- 修正 AI 操作目标边界：用户提“章纲、节点安排、小节目标、本章结构”时优先更新当前章纲；提“大纲、全书大纲、分卷、主线规划”时更新全书大纲；`target=chapter` 仅表示正文，不再误把章纲修改写进正文。
- 增加当前章纲持久显示与同步：生成章纲后写入 daemon state，SSE 收到 `outline_ready` 时同步到前端；右侧支持显示章纲覆盖文本和完整章纲折叠区。
- 强化回滚反馈：换一版时记录被拒稿件、拒绝原因和上一版摘录，并在右侧“本轮改写要求”中显示，下一版会避免沿用旧开头、节奏、意象和叙述重心。
- 修复多处写作状态问题：修复增强章纲返回 dict 后守护进程仍按 list 迭代导致卡住的问题；关闭终端后再次启动会把保存的 `running` 状态归一化为 `idle/stopped`，不再假装后台仍在写作；处理 `no pending review node`/`409 Conflict` 时前端会刷新状态并清理过期待审编辑。
- 增强正文风格门禁：节点生成加入反文艺腔、反散文腔检测和自动重写，降低“红烛、珠帘、命运、仿佛”等漂亮废话；同时增加反重复和网文节奏检查，要求动作、对话、信息、冲突、代价更直接。
- 新增“重写本章”流程：可清空当前章正文、节点草稿、锁定节点、待审状态和进度，并自动重新启动写作；按钮移动到审稿按钮组首位，不挤占正文编辑空间。
- 修复并稳定审阅闭环：待审节点新增 `revision_id` 和 `attempt_no`；写入、改后写入、换一版都会校验当前 revision，防止过期按钮误操作；前端不再自行把待审正文追加进章节正文，章节正文以服务端 state 为准。
- 修复保存状态回滚硬错误：持久化待审回滚路径不再在 `chapter_index` / `node_index` 定义前使用变量。
- 章纲覆盖真正进入生成器：`chapter_outline:N` 不再只用于右侧显示，而是作为作者指定章纲注入当前章规划。
- 修正章纲默认节奏：退婚、打脸、压迫、反套路、夺运等场景的第一节点不再默认为氛围铺垫，而是优先 `dialogue_push` / `action_beat`，要求开头三句内写清谁来、要做什么、谁在阻拦。
- 新增节点级质量门禁：小节草稿进入待审前会检查文艺腔、网文节奏、开头冲突、意象堆砌、动作/对话/冲突密度和重复问题，不合格会自动带问题重写一次。
- 回滚类型化：待审区新增“文风、节奏、角色、章纲、重复”原因按钮和自定义原因输入；后端会把原因翻译成具体重写约束，让“换一版”不再只是抽卡。
- 增加章纲 dirty 和案头 dirty 机制：修改当前章纲会提示旧正文/旧小节可能不匹配；修改普通案头设定会提示当前章纲可能过期。
- 右侧 dirty 提示增加可执行动作：可直接“按新设定重写本章”“按新章纲重写当前小节”或“重写本章”。
- 新增“只重做章纲”能力：新增 `POST /api/v1/daemon/regenerate-outline`，可只刷新当前章纲，不清正文、不删小节、不自动写正文；前端右侧 dirty 提示中加入“只重做章纲”按钮。
- 增加作者写作进度卡、节点状态持久化和恢复逻辑，改进续写时从当前章/下一章继续的判断，减少重启或暂停后写作进度错位。
- 增加书稿下载侧栏，并进一步压缩写作空间交互，让主界面更偏向写正文和审稿，而不是监控面板。
- 在正式深度重构前创建 GitHub 检查点 `b89a6ee chore: checkpoint before writing workflow refactor`；之后本地新增检查点 `94adf05 refactor: stabilize writing review and outline flow`，但由于连接 GitHub 失败，最新本地提交尚未推送到远程。
- 这是写作闭环、章纲生成、回滚反馈和正文同步的局部功能升级，按版本号规则递增第二位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.1.0`。
- 验证通过：多轮执行 `python -m compileall -q storyforge` 与 `npm run build`。

## 2026-05-05 · v2.0.9

- 查询并参考 GitHub 上的开源 UI 图标库方向：Lucide 提供 1600+ SVG 图标和 Vue 包，但本轮为避免新增依赖，采用本地 CSS/SVG 等价物继续实现。
- 重做全局工作台背景为深色木纹桌面，降低纯黑背景与纸张的割裂感。
- 将工作台纸面进一步统一为两页连续的干净米白色纸张，减少淡黄色卡片割裂感。
- 纸张横线改为更像真实稿纸的墨线，并进一步弱化独立卡片边界，让审稿和 AI 编辑更像写在纸上的栏目。
- 左侧导航按钮改为木质/古铜徽章质感，图标使用印章式金色深度样式，减少扁平互联网感。
- 顶部按钮改为浮雕陶瓷/金属牌质感，章节书签改为更真实的纸签/牛皮纸标签。
- Skill 标签改为近似纸上印章/手工纸条的样式。
- 在正文页加入一组 CSS 绘制的钢笔和笔帽道具，模拟参考图中的物理书写工具，但未引入外部图片资源。
- 这是视觉素材与拟物细节修正，按版本号规则递增第三位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.0.9`。
- 验证通过：`npm run build`。

## 2026-05-05 · v2.0.8

- 按用户要求进一步贴近参考图，将写作工作台重置为更明确的三列活页本结构：左页 / 独立活页环槽 / 右页。
- 强制移除旧的中缝、卷角和左页环扣伪元素，解决多轮样式叠加造成的环扣覆盖文字问题。
- 活页环改为只依附在左右页之间的装订槽附近，不再伸入左页内容区。
- 纸张颜色和字体进一步向参考图收敛：更接近浅灰白纸、细横线稿纸和较正式的宋体系标题。
- 右页正文、页边批注、顶部按钮和章节书签重新压缩为参考图式比例，减少装饰性偏差。
- 这是视觉对齐与遮挡修复，按版本号规则递增第三位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.0.8`。
- 验证通过：`npm run build`。

## 2026-05-05 · v2.0.7

- 修复写作工作台中活页环覆盖左页文字和卡片内容的问题。
- 左页增加内侧安全留白，并缩小/右移活页环装饰，避免装饰元素压住案头设定、审稿与 AI 编辑区域。
- 重新分配打开书的左右页比例，让右页正文区域更宽，减少页边批注和正文之间的拥挤。
- 收紧左页案头设定、审稿区和 AI 编辑区高度，长内容在各自区域内部滚动，避免继续向下堆叠。
- 略微缩小标题、正文和页边批注卡片尺寸，降低字符遮挡概率。
- 这是布局遮挡修复，按版本号规则递增第三位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.0.7`。
- 验证通过：`npm run build`。

## 2026-05-05 · v2.0.6

- 按用户给出的参考图，将写作工作台进一步调整为更接近真实打开的活页笔记本。
- 书页比例、纸色、边框和阴影改为更克制的写实纸张效果，减少旧版过黄、过圆、过装饰的问题。
- 中缝重新处理为金属活页夹效果，加入纵向金属脊和一组环形孔扣，弱化抽象书脊阴影。
- 右页正文区域改成更像真实稿纸的细线背景，页边批注卡片改为白纸便签式阴影卡片。
- 顶部操作按钮和审阅按钮进一步收窄、降低圆角，贴近参考图里的工具按钮形态。
- 章节书签调整为更浅的纸签样式，位置更接近参考图右侧标签。
- 这是视觉对齐修正，按版本号规则递增第三位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.0.6`。
- 验证通过：`npm run build`。

## 2026-05-05 · v2.0.5

- 修复工作台打开书界面局部堆叠和溢出问题。
- 左页案头设定、当前审稿和 AI 编辑改为固定头部 + 可滚动内容的高度分配，避免审稿区和 AI 对话区互相压住。
- 当前审稿文本框限制在合理高度内滚动，审阅按钮保持在卡片下方，不再被长文本挤出页面。
- AI 编辑区限制聊天流和输入框高度，避免输入框从书页底部溢出。
- 右页标题和操作按钮固定高度，四个操作按钮使用紧凑网格，减少与页边批注区域重叠。
- 右页正文和页边批注区域改为受控高度，页边批注独立滚动，底部字数状态栏不再被内容覆盖。
- 章节书签位置略微下移并收窄，减少与页边批注和页面边缘冲突。
- 这是布局修复，按版本号规则递增第三位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.0.5`。
- 验证通过：`npm run build`。

## 2026-05-05 · v2.0.4

- 继续按后续视觉建议增强“创作笔记本”的物理质感。
- 活页/线装孔位增加金属环高光、阴影与向纸张投射的细影，弱化扁平色块感。
- 书架背景去掉偏数字化的扫描线感，改为更大块、更低对比的深色木纹，并增加层板厚度、反射光和环境遮蔽阴影。
- 书本增加底部/后方 AO 阴影，让本子更像立在书架层板上，而不是贴在背景墙上。
- 纸页边缘增加微弱亮边、内侧纸边阴影和轻微卷角/磨损暗示。
- 红笔圈阅与划线痕迹改用更低透明度与 multiply 混合，模拟墨水渗进纸面。
- 这是视觉质感修正，按版本号规则递增第三位。
- 同步版本号：前端 [`package.json`](storyforge/frontend/package.json)、后端 [`pyproject.toml`](storyforge/pyproject.toml)、FastAPI 版本与页面 `APP_VERSION` 均提升至 `2.0.4`。
- 验证通过：`npm run build`。
